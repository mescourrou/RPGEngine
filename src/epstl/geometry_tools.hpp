...
class Point
{
  public:
    ...
    template<typename = std::enable_if_t<(k_size >= 1) >> k_type& x() noexcept
    {
        return m_coordinates[0];
    }
    /// @brief Get and modify the y coordinate (1)
    template<typename = std::enable_if_t<(k_size >= 2) >> k_type& y() noexcept
    {
        return m_coordinates[1];
    }
    /// @brief Get and modify the z coordinate (2)
    template<typename = std::enable_if_t<(k_size >= 3) >> k_type& z() noexcept
    {
        return m_coordinates[2];
    }
    /// @brief Get the x coordinate (0)
    template<typename = std::enable_if_t<(k_size >= 1) >> k_type x() const noexcept
    {
        return m_coordinates[0];
    }
    /// @brief Get the y coordinate (1)
    template<typename = std::enable_if_t<(k_size >= 2) >> k_type y() const noexcept
    {
        return m_coordinates[1];
    }
    /// @brief Get the z coordinate (2)
    template<typename = std::enable_if_t<(k_size >= 3) >> k_type z() const noexcept
    {
        return m_coordinates[2];
    }
