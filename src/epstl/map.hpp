...
public:
	map() = default;
	map(bool (*less_operator)(const key_t& k1, const key_t& k2)) : m_less_operator(less_operator) {}
	~map() override;
	...
	epstl::size_t height() const noexcept { return height(m_root); }
...
template<typename key_t, typename item_t>
bool map<key_t, item_t>::insert_recursive(node_t* current_node, key_t& key, item_t& item) noexcept
{
...
template<typename key_t, typename item_t>
auto map<key_t, item_t>::search(const key_t &key) const noexcept -> map<key_t, item_t>::node_t*
{
